(function($) {
    ...
    /*changeModule = (val) => {
                                  var url = $("#url").val();
                                  var module = $(".module_switch").data('id');
                                  alert(val);

                                  $("#ch"+module).click(function (){
                                     alert('clicked');
                                  });
                                  /!*$.ajax({
                                      type: "GET",
                                      ...
                                      beforeSend: function( xhr ) {
                                          $("#ch"+module).html('<button class="primary-btn fix-gr-bg"></span>Acivating.....</button>')
                                      },
                                      url: url + "/" + "manage-adons-enable/" + val,
                                      success: function(data) {
                                          $("#ch"+module).html('<label class="switch"><input type="checkbox" data-id='+module+ 'id="ch"'+module+ 'onclick="changeModule(`+`)" class="switch-input1 module_switch" {{@$row->isDisabled() == true? checked}}> <span class="slider round"></span> </label>');
                                          if (data["success"]) {
                                              if (data["data"] == "enable") {
