...
class SmStudentAdmissionController extends Controller
{
    ...
    public function studentDetailsSearch(Request $request)
    {
        ...
        try {
            // if ($request->name)
            //     $students = SmStudent::with('className','parents','section','gender','category')->where('active_status', 1)->where('class_id', $request->class)
            ...
            $students = $students->with('className','section','parents','section','gender','category')->where('school_id', Auth::user()->school_id)->get();
