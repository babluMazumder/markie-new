...
class SmAcademicsController extends Controller
{
    ...
    public function classReportSearch(Request $request)
    {
        ...
        try {
            ...
            $assign_subjects->where('academic_id', getAcademicId())->where('active_status', 1);
            if ($request->section != "") {
            ...
            foreach ($students as $student) {
                $fees_assigns = SmFeesAssign::where('academic_id', getAcademicId())->where("student_id", $student->id)->where('active_status', 1)->get();
                foreach ($fees_assigns as $fees_assign) {
                    $fees_masters = SmFeesMaster::where('academic_id', getAcademicId())->where('id', $fees_assign->fees_master_id)->get();
                    foreach ($fees_masters as $fees_master) {
                        $total_collection = $total_collection + SmFeesPayment::where('academic_id', getAcademicId())->where('student_id', $student->id)->where('fees_type_id', $fees_master->fees_type_id)->sum('amount');
                    }
                }
                ...
                $applied_discount+=SmFeesAssign::where('student_id',$student->id)->sum('applied_discount');
            }

            $classes = SmClass::where('academic_id', getAcademicId())->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
           
            return view('backEnd.reports.class_report', compact('classes', 'students','applied_discount', 'assign_subjects', 'assign_class_teachers', 'total_collection', 'total_assign', 'search_class', 'section'));
        } catch (\Exception $e) {
            // dd($e);
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
