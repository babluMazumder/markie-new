...
class SmFeesController extends Controller
{
    ...
    public function feesGenerateModal(Request $request, $amount, $student_id, $type,$master)
    {
        ...
        try {
            ...
            $discounts = SmFeesAssignDiscount::where('student_id', $student_id)->where('fees_type_id', $fees_type_id)->where('school_id',Auth::user()->school_id)->first(); 
            // dd($discounts);
            // $applied_discount = [];
            ...
            return view('backEnd.feesCollection.fees_generate_modal', compact('amount','master', 'discounts', 'fees_type_id', 'student_id', 'data', 'method'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function feesPaymentStore(Request $request)
    {
        ...
        //     'amount' => "required|max:real_amount"
        // ]);
        try {
// return $request;
            $fileName = "";
    ...
    public function feesPaymentDelete(Request $request)
    {
        try {
                if (checkAdmin()) {
                    ...
                    $result = SmFeesPayment::where('id',$request->id)->where('school_id',Auth::user()->school_id)->delete();
                }
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                if ($result) {
    ...
    public function feesDueSearch(Request $request)
    {
        ...
        try {
            ...
            foreach ($students as $student) {
                ...
                $amount = SmFeesPayment::where('student_id', $student->id)->where('academic_id', getAcademicId())->where('fees_type_id', $fees_group[1])->sum('amount');
        ...
        try {
            // $payment = SmFeesPayment::find($id);
             if (checkAdmin()) {
                    ...
                    $payment = SmFeesPayment::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
                }
            $group = $group;
...
// return  $fees_assigneds;
    ...
    public function bankPaymentSlipSearch(Request $request)
    {
        ...
        try {
            ...
            // dd($clsses);
            // $bank_slips = SmBankPaymentSlip::where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->orderBy('id', 'desc')->get();
    ...
    // public function bankPaymentSlipSearch(){
    //     $bank_slips = SmBankPaymentSlip::where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->orderBy('id', 'desc')->get();
